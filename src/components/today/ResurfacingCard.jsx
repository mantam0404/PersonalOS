...
export function ResurfacingCard() {
  ...
  return (
    <section className="space-y-3">
      ...
      {!item ? (
        <div className="rounded-xl border border-dashed border-slate-300 dark:border-slate-700 bg-slate-100 dark:bg-slate-900/50 p-5 text-center">
          ...
          {isAdding ? (
            <form onSubmit={handleAddHighlight} className="mt-3 space-y-2">
              <textarea
                ...
                rows={3}
                autoFocus
                className="w-full rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-200 dark:bg-slate-800 px-3 py-2 text-sm text-slate-900 dark:text-slate-100 outline-none focus:border-purple-500"
              />
              <button
                type="submit"
            ...
            <button
              ...
              className="mt-3 rounded-lg border border-slate-300 dark:border-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800"
            >
              + 新增重點筆記
            </button>
          )}
        ...
        >
          ...
          {item.content && (
            <p className="mt-2 text-sm leading-relaxed text-slate-600 dark:text-slate-400">{item.content}</p>
          )}
          {!isReviewed && (
            <button
              type="button"
